	...
	for i=1, #defaultSpells1 do
		...
		if name then
			DefaultSettings.profile.Spells[spellID] = {
				...
				spellID = spellID,
				scale = 1.75,
				durationSize = 16, --seems like the player cant change this value ingame (the setting has no effect), so setting it here
...
local function FormatTime(seconds)
	if seconds < 10 and db.showDecimals then -- CHECK THIS KHAL (this used to be 10 seconds(it still shows as "10" in the ingame options but im too lazy to edit it there so cba... ...... 10->1)
		return "%.1f", seconds
	...
	if total > 0 then
		...
		if self.expiration and self.expiration > 0 then
		   ...
		   -----------------------------------------------------
			...
			if self.duration > 6 then
				local bth = 1
				local fth = 1
				  if timeLeft < (db.blinkThreshold + 1/3) then
					bth = timeLeft % 1 
					if bth > 0.5 then
						bth = 1 - bth
					end
					bth = math.min(math.max(bth * 3, 0), 1)
				end
				if timeLeft < db.fadeThreshold then
...
local function CreateBuffIcon(frame,i)
	...
	------- Radial Clock Texture, Bugs when anchored to nameplate --------
	...
	--------------- Vertical Clock Texture, by Khal ----------------
	...
	if MSQ then
		Group:AddButton(buffIcon,{
			...
			--Cooldown = buffIcon.cooldown, -- by Khal
	...
	for _,p in pairs(flyPlateBuffsDB.profiles) do
		if p.Spells then
			...
			for n,s in pairs(p.Spells) do
				...
				if not spellID then
					for i=1, #defaultSpells3 do
						if n == GetSpellInfo(defaultSpells3[i]) then
				...
				if not spellID then
					for i=1, #defaultSpells4 do
						if n == GetSpellInfo(defaultSpells4[i]) then
							spellID = defaultSpells4[i]
							break
						end
					end
				end
